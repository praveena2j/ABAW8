...
class GRJCA_cam(nn.Module):
    def __init__(self):
        super(GRJCA_cam, self).__init__()
        ...
        self.audio_gating_fc_layer = nn.Linear(128, 3)
        self.softmax = nn.Softmax(dim=-1)
                                    
        self.video_gating_fc_layer = nn.Linear(256, 3)
        self.text_gating_fc_layer = nn.Linear(128, 3)
    ...
    def forward(self, f1_norm, f2_norm):
        ...
        video_weights = self.video_gating_fc_layer(atten_video[-1])
        #print(video_weights.shape)
        ...
        audio_weights = self.audio_gating_fc_layer(atten_audio[-1])
        ...
        #text_weights = self.text_gating_fc_layer(atten_text[-1])
